...
abstract class eZViewCounterFactory {
    ...
    private static function buildeZViewCounterFile(eZPersistentObject $node) {
        ...
        if ( !($eZViewCounter instanceof eZViewCounterFile && $node) ) {
            // first step : $node_id = $node->getID();
            // second step : $eZViewCounter = eZViewCounterFile::create($node_id);
            // third step : $eZViewCounter->store();
        }
        return $eZViewCounter;
    }
